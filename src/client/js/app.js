...
const weatherbitBaseUrlCurrent = process.env.WEATHERBIT_URL_CURRENT;
const weatherbitBaseUrlForecast = process.env.WEATHERBIT_URL_FORECAST;
...
const fetchWeather = async function (lat, lon, userDate) {
  ...
  if (dateDiff > 1382400000) {
    ...
    let weatherbitResponse = await fetch(
      weatherbitBaseUrlCurrent +
        "lat=" +
        lat +
        "&lon=" +
        lon +
...
async function updateWeather(weatherInfo) {
  ...
  feelsDiv.innerHTML = `${
    weatherInfo.app_temp ? weatherInfo.app_temp + "C" : "-"
  }`;
...
function findDateInApiResponse(travelDate, apiResponse) {
  ...
  const foundDate = apiResponse.find(
    (obj) => travelDateForAPI === obj.valid_date
  );
...
function convertTimezone(userDate, weatherbitInfo) {
  ...
  sunriseTime = sunriseTime.toLocaleTimeString("en-GB", {
    timeZone: weatherbitInfo.data[0].timezone,
  });
  sunsetTime = sunsetTime.toLocaleTimeString("en-GB", {
    timeZone: weatherbitInfo.data[0].timezone,
  });
